(function () {
    ...
    angular
        .module('axonApp')
        .controller('GraphEditorController', ['compileService', '$routeParams', '$location', '$scope', modelController]);

    function modelController(compileService, $routeParams, $location, $scope) {
        var layerTypes = {
            "Input" : ["dimensions"], 
        ...
        $scope.graph.containers =  [ {"items" : [{"name" : "Input", "opts" : ["dimension"], "input" : {}}], "class" : "layers"}, 
                {"items" : [], "class" : "sidebar"}];
        ...
        $scope.graph.compile = function() {
            ...
            compileService.gen(angular.toJson(formatLayers($scope.graph.containers[0].items)));
        };
